%% SPRiNT (without Brainstorm) pipeline
perso = {'liberal', 'conservative', 'default', 'natural'};
chans = 5000;
for mode_n = 3 %1:length(perso)
    mode = perso{mode_n};
    % Inputs
...
%     file_name = ['Results/' mode num2str(chans) '.mat'];
%     save(file_name,'mode','mAE_cell','stats','stats_mat', 'rstmp', 'vstmp', 'ostmp');
%     file_name_spectra = ['Results/SPECTRA' mode num2str(chans) '.mat'];
%     spectra = [s_data.SPRiNT.channel(:).stats];
%     save(file_name_spectra,'spectra'); %second file
%     clear SPRiNT  s_data outputStruct channel Freqs TF ts file_name opt mAE_cell stats stats_mat
end
...
% Author: Luc Wilson (2022)
    ...
    parfor chan = 1:nChan
        ...
        channel(chan).aperiodics(nTimes) = struct(...
            ...
            'offset',           [],...
            'exponent',         []);
        channel(chan).stats(nTimes) = struct(...opt.peak_threshold
            'MSE',              [],...
            'r_squared',        [],...
