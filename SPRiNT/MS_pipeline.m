% Ben's Composite SPRiNT pipeline
perso = {'natural', 'default', 'conservative'};
chans = 25;
for mode_n = 1 %:length(perso)
    mode = perso{mode_n}
    sfreq = 200;
    F = open('sim_FULL.mat').xs;
    F = F(1:chans,:);
    RowNames = 1:size(F,1);
    ...
    % @=============================================================================
    % This function is NO LONGER part of the Brainstorm software:
    % https://neuroimage.usc.edu/brainstorm
    % Authors: Luc Wilson (2021)
    % =============================================================================@
    
    % Fetch user settings THE VALUES HAVE BEEN FIXED HERE FOR TESTING
    opt = struct();
    ...
    opt.hOT = 0;
    opt.optim_obj           = 'negloglike'; % negloglike or leastsquare
    ...
    % Find all frequency values within user limits
        ...
        parfor chan = 1:nChan
            ...
            for time = 1:nTimes
                ...
                try
                [est_pars, pk_function] = est_peaks(fs, flat_spec, opt.max_peaks, opt.peak_threshold, opt.min_peak_height, ...
                ...
                for pk = 0:size(est_pars,1)
                    peak_pars = est_fit(est_pars(1:pk,:), fs, flat_spec, opt.peak_width_limits/2, opt.peak_type, opt.guess_weight,opt.hOT);
                    % Refit aperiodic
                    aperiodic = spec(time,:);
                    for peak = 1:size(peak_pars,1)
                        aperiodic = aperiodic - pk_function(fs,peak_pars(peak,1), peak_pars(peak,2), peak_pars(peak,3));
                    end
                    aperiodic_pars = simple_ap_fit(fs, aperiodic, opt.aperiodic_mode,aperiodic_pars(2));
                    guess = peak_pars;
                    if ~isempty(guess)
        ...
        %                 ub = [guess(1:pk,1)+guess(1:pk,3)*2,inf(size(guess(1:pk,2))),ones(size(guess(1:pk,3)))*opt.peak_width_limits(2)/2]';
                    ...
                    try
                        params = fmincon(@err_fm_constr,guess,[],[],[],[], ...
                            lb,ub,[],options,fs,spec(time,:),opt.aperiodic_mode,opt.peak_type);
                    catch
                        a = 0;
                    end
                    switch opt.aperiodic_mode
                    ...
                    %as far as BF goes, what does this mean? Why does the first
                    %model of everything have a BF of 1? is it becaues BF is
                    %relative? yes relative to h0, ours is flipped st small is
                    %better
                end
                %picks the best model based off of BIC
    
                %finds index of the model that has the lowest BIC
                [trash,n] = find([model.BIC] == min([model.BIC]));
                
                aperiodic_pars(2) = abs(aperiodic_pars(2));
                channel(chan).data(time).time                = ts(time);
                channel(chan).data(time).aperiodic_params    = model(n).aperiodic_params;
                channel(chan).data(time).peak_params         = model(n).peak_params;
                channel(chan).data(time).peak_types          = func2str(peak_function);
                channel(chan).data(time).ap_fit              = 10.^ap_fit;   %NEEDS MODIFICATION
                aperiodic_models(chan,time,:)                = 10.^ap_fit;   %NEEDS MODIFICATION
                channel(chan).data(time).fooofed_spectrum    = 10.^model_fit;   %NEEDS MODIFICATION
                SPRiNT_models(chan,time,:)                   = 10.^model_fit;   %NEEDS MODIFICATION
                channel(chan).data(time).power_spectrum   	 = 10.^spec(time,:); %NEEDS MODIFICATION
                channel(chan).data(time).peak_fit            = 10.^(model_fit-ap_fit); %NEEDS MODIFICATION
                peak_models(chan,time,:)                     = 10.^(model_fit-ap_fit); %NEEDS MODIFICATION
                channel(chan).data(time).error               = model(n).MSE;
                channel(chan).data(time).r_squared           = model(n).r_squared;
                channel(chan).data(time).loglik             = model(n).loglik; % log-likelihood
                channel(chan).data(time).AIC                = model(n).AIC;
                channel(chan).data(time).BIC                = model(n).BIC;
                channel(chan).data(time).models             = model;
                % Extract peaks
                if ~isempty(model(n).peak_params) & any(model(n).peak_params)
                    for p = 1:size(model(n).peak_params,1)
                        channel(chan).peaks(i).time = ts(time);  
                        channel(chan).peaks(i).center_frequency = model(n).peak_params(p,1); 
                        channel(chan).peaks(i).amplitude = model(n).peak_params(p,2); 
                        channel(chan).peaks(i).st_dev = model(n).peak_params(p,3); 
                        i = i +1;
                    end
                end
                % Extract aperiodic
                channel(chan).aperiodics(time).time = ts(time);   
                channel(chan).aperiodics(time).offset = model(n).aperiodic_params(1);   
                if length(aperiodic_pars)>2 % Legacy FOOOF alters order of parameters
                    channel(chan).aperiodics(time).exponent = model(n).aperiodic_params(3);
                    channel(chan).aperiodics(time).knee_frequency = model(n).aperiodic_params(2);
                else
                    channel(chan).aperiodics(time).exponent = model(n).aperiodic_params(2);
                end
                channel(chan).stats(time).MSE = model(n).MSE;
                channel(chan).stats(time).r_squared = model(n).r_squared;
        ...
        if strcmp(opt.rmoutliers,'yes')
            ...
            SPRiNT = BICrecalculator(SPRiNT, fs)
        end
        for chan = 1:nChan
    ...
    %Code that will give us an output similar to MATLAB SPRiNT
        s_data = struct('Freqs', fs, 'SPRiNT', SPRiNT);
    %Pipeline
    [stats,stats_mat, mAE_cell, rstmp, vstmp, ostmp] = model_q(s_data,chans);
%     file_name = ['MS' mode num2str(chans) '_2.mat'];
%     save(file_name,'mode','mAE_cell','stats','stats_mat', 'rstmp', 'vstmp', 'ostmp'); %first file
%     file_name_spectra = ['SPECTRAMS' mode num2str(chans) '2.mat'];
%     spectra = [SPRiNT.channel(:).stats];
%     save(file_name_spectra,'spectra'); %second file
%     clear  s_data outputStruct channel Freqs TF ts file_name opt mAE_cell stats stats_mat ...
%             Ffft fMask FreqVector Fwin indGood iTimes iWin Loverlap Lwin Nwin  tp_offset ... 
%             tp_exponent TFwin RowNames sens TFfull  Win* peak* n* fs NFFT Messages aperiodic_models center_time ...
%             OPTIONS SPRiNT_models TFtmp rstmp vstmp SPRiNT ostmp %cleanup before next iteration
end
