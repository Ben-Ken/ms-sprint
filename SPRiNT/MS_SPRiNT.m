%% MS-SPRiNT 
% by Benjamin Levesque Kinder, 2023
% adapted from SPRiNT, Wilson et al., 2022
% =======================================================================

chans = 25; %set the number of channels
sfreq = 200; %set the sampling frequency

F = open('sim_FULL.mat').xs; %import data
RowNames = 1:size(F,1);
...
%Set peak post-processing parameters. Some presets are provided, otherwise
%use 'custom'.
mode = 'natural'
...
switch mode
    case 'natural'
        opt.rmoutliers          = 'yes';
        opt.maxfreq             = opt.freq_range(2); %freq range
        opt.maxtime             = 120; %length of data in s, divided by window length
        opt.minnear             = 0; 
    case 'default'
        opt.rmoutliers          = 'yes';
        opt.maxfreq             = 2.5;
...
% Find all frequency values within user limits
    ...
    %                 ub = [guess(1:pk,1)+guess(1:pk,3)*2,inf(size(guess(1:pk,2))),ones(size(guess(1:pk,3)))*opt.peak_width_limits(2)/2]';
            ...
            [ignore_this,n] = find([model.BIC] == min([model.BIC]));
            
            %data with the 'NEEDS MODIFICATION' tag uses data from model
            %with most peaks rather than lowest BIC. This is not an issue
            %because these data are recalculated by the BIC recalculator.
    ...
    if strcmp(opt.rmoutliers,'yes')
        SPRiNT = remove_outliers(SPRiNT,peak_function,opt); %remove outliers
    end
    SPRiNT = BICrecalculator(SPRiNT, fs) %recalculate BIC and aperiodic data
    for chan = 1:nChan
    ...
    s_data = struct('Freqs', fs, 'SPRiNT', SPRiNT); %formatting
